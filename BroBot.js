...
uploadTable.forEach(item => {
    if(message.startsWith(item[0])) {
        fs.readdir(item[1], (err, files) => {
            let file = files[Math.floor(Math.random() * files.length)];
            let filename = item[1] + file;
            channel.uploadFile(filename, filename);
        })
    }
});
	...
	if (/:([\w]+):/g.test(S(message)) == true) {
		...
		author.openDM().then(function(dm) {
			dm.sendMessage("You can view all of Something Awful's emotes by following this link: http://discordgoons.com/sa-emotes.html");
		});
	}
	...
	/*else if (/!thinking/g.test(S(message)) == true) {
		...
		replies = ["Obviously", "It is certain", "It is decidedly so", "Without a doubt", "Yes, definitely",
		"You may rely on it", "As I see it, yes", "Most likely", "Outlook seems good", "Yes", "Signs point to yes",
		"Don't count on it", "My reply is no", "My sources say no", "Outlook is not so good", "Seems doubtful",
		"All signs point to no", "Most definitely not", "Most likely... no", "Do not rely on it", "Why are you asking me this?", "Fuck off, fag"];
	...
	*/
		...
		if(author.can(Discordie.Permissions.General.MANAGE_GUILD, guild) ) {
			...
			channel.sendMessage(":no_entry_sign: `you do not have permissions to do that` :no_entry_sign:");
		}
	}

	else if (message === "!invite") {
		if(author.can(!Discordie.Permissions.General.CREATE_INSTANT_INVITE, guild) ) {
			channel.sendMessage(":no_entry_sign: `I do not have permissions to do that` :no_entry_sign:")
		} else{
			var generateInvite = channel.createInvite({"temporary": false, "xkcdpass": false});
			generateInvite.then( function(res) { channel.sendMessage("https://discord.gg/" +res.code); });
		}
	}
